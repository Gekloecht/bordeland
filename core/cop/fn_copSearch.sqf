...
if(count _invs > 0) then
{
	...
	[[0,format["%1 a €%2 de valeur de drogue / contrebande on them.",name _civ,[_illegal] call life_fnc_numberText]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
}
	else
{
	_inv = "Aucun item illegal";
};
if(!alive _civ || player distance _civ > 5) exitWith {hint format["Echec de la recherche %1", name _civ]};
//hint format["%1",_this];
...
,name _civ,_license,_inv,if(_robber) then {"Vole la banque"} else {""}];

if(_robber) then
{
	[[0,format["%1 a ete identifie comme le braqueur de banque!",name _civ]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
};