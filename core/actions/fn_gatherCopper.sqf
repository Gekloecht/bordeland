...
_sum = ["copperore",_rand,life_carryWeight,life_maxWeight] call life_fnc_calWeightDiff;
if (life_inv_pickaxe < 1) exitWith {hint "Vous avez besoin d'une pioche pour effectuer cette action."};
if(_sum > 0) then
{
	titleText["Récolte de cuivre...","PLAIN"];
	titleFadeOut 5;
	player say3D "mining";
	...
	{
		titleText[format["Vous avez collecté %1 cuivre",_sum],"PLAIN"];
	};
};
