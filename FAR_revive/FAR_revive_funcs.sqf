...
if(side _killer != west && alive _killer) then
{
			[[getPlayerUID _killer,name _killer,"500"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;	
};
	...
	if (isPlayer _unit) then 
	{
		...
		{
			hintSilent format["Hémorragie, mort dans %1 secondes\n\n%2", round (_bleedOut - time), call FAR_CheckFriendlies];
		...
		if (_unit getVariable "FAR_isStabilized" == 1) then {
			...
			{
				hintSilent format["Vous êtes inconscient\n\n%1", call FAR_CheckFriendlies];
...
FAR_CheckFriendlies =
{
	...
	if (count _medics > 0) then
	{
		...
		if (!isNull _unit) then
		{
			...
			_hintMsg = format["Médecin à proximité:\n%1 est à %2m", _unitName, _distance];
		};
	} 
	else 
	{
